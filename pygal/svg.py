...
class Svg(object):
    ...
    def node(self, parent=None, tag='g', attrib=None, **extras):
        ...
        for key, value in dict(attrib).items():
            if value is None:
                del attrib[key]
            if isinstance(value, Number) and type(value) != int:
                value = round(value, 2)
    ...
    def serie(self, serie):
        ...
        return dict(
            plot=self.node(
                ...
                class_='series serie-%d color-%d' % (serie.index, self.graph.color_classes[serie.index-1]
                                                     if self.graph.color_classes else serie.index)
            ),
            overlay=self.node(
